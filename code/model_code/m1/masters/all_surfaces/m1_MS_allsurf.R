#========== This script file runs M1 on the combined surface data set for the Masters Tour ==========

rm(list=ls()) #clearing environment
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Model_fitting_home_computer/fixed_time_bt/master/master_allsurf")
...
#importing the data
atp_matches_2000_BT <- read.csv2("atp_matches_2000_train_MS_allsurf.csv",header = TRUE)
atp_players_2000_BT <- read.csv2("atp_players_2000_train_MS_allsurf.csv",header = TRUE)
...
t2 <- Sys.time()
run_time <- t2-t1
save(out,model.data,run_time,file = "BT_fixed_time_out_MS_allsurf.RData")
...
t11 <- Sys.time() #used to obtain the run time of the algorithm
for (i in 1:4){
  out <- update(out,parameters.to.save = parms, n.iter = 15000,n.thin = 3)
  save(out,model.data,file = "BT_fixed_time_out_MS_allsurf_with_extra_burnin_2.RData")
  print(i)
}
t21 <- Sys.time()
run_time_2 <- t21-t11
save(out,model.data,run_time_2,file = "BT_fixed_time_out_MS_allsurf_with_extra_burnin_2.RData")
#===========================================================================================================