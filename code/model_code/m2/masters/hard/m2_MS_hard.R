#========== This script file runs M2 on the hard data set for the Masters Tour ==========

#Clearing environment and setting working directory
rm(list = ls())
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Model_fitting_home_computer/gorgi_noreg_nosurf/master/master_hard")
...
model <- "model{
  ...
  for (i in 1:n_players){
    for (t in 2:n){
    #Check that we are using all the matches with the indices.
...
#saving output
save(out,model.data,run_time,file = "gorgi_simple_model_output_MS_hard.RData")
...
for (i in 1:8){
  ...
  save(out,model.data,file = "gorgi_no_reg_nosurf_MS_hard_with_extra_burnin.RData")
  print(i)
}
t21 <- Sys.time()
run_time_2 <- t21-t11
save(out,model.data,run_time_2,file = "gorgi_no_reg_nosurf_MS_hard_with_extra_burnin.RData")
#===========================================================================================================
