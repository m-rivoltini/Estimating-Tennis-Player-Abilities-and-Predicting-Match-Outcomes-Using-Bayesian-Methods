# ================= Analysing outputs of M2 applied to the clay data set for Masters Tour ===========
...
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Analysing_model_fit_outputs/gorgi_noreg_nosurf/master/master_clay")

#loading outputs and obtained run times for each compilation
load(file = "/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_clay/gorgi_simple_model_output_MS_clay.RData")
run_time

load(file = "/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_clay/gorgi_no_reg_nosurf_MS_clay_with_extra_burnin.RData")
run_time_2

#importing data 
atp_matches_2000_train <- read.csv2("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_clay/atp_matches_2000_train_MS_clay.csv")
atp_players_2000_train <- read.csv2("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_clay/atp_players_2000_train_MS_clay.csv")
...
#saving summaries
write.csv(summaries,file = "M2_MS_clay_summaries.csv")
...
pdf("M2_MS_clay_seq_plots_top_players_ordered.pdf")
par(mfrow = c(3,1))
...
pdf("M2_MS_clay_lambda_trace_plots.pdf")
par(mfrow = c(3,1))
...
pdf("M2_MS_clay_lambda_density_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_MS_clay_p_ij_trace_plots.pdf")
par(mfrow = c(3,1))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_clay_p_ij_density_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_clay_lambda_acf_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_MS_clay_p_ij_acf_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_clay_lambda_gelman_plots.pdf")
#par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_MS_clay_p_ij_gelman_plots.pdf")
#par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_clay_tau_trace_plots.pdf")
coda::traceplot(out$samples[,"tau"])
dev.off()

pdf("M2_MS_clay_tau_density_plots.pdf")
coda::densplot(out$samples[,"tau"])
dev.off()

pdf("M2_MS_clay_tau_acf_plots.pdf")
acf(out$sims.list$tau,main = "tau")
dev.off()

pdf("M2_MS_clay_tau_gelman_plots.pdf")
...
#====================checking convergence and finding which player strengths / probabilities did not converge========================= - note used
...
save(BIC,file = "M2_MS_clay_BIC.RData")
