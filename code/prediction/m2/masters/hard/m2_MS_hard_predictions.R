#This script file performs the predictions for out-of-sample matches for M2 on hard surface (Masters Tour)
...
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Predictions/gorgi_noreg_nosurf/master/master_hard")

#loading data sets
atp_matches_2000_train <- read.csv2(file = "atp_matches_2000_train_MS_hard_reduced_2.csv",header = TRUE)
atp_players_2000_train <- read.csv2(file = "atp_players_2000_train_MS_hard_reduced_2.csv",header = TRUE)
...
atp_matches_2000_train_test <- read.csv2(file = "atp_matches_2000_train_test_MS_hard.csv",header = TRUE)
atp_players_2000_train_test <- read.csv2(file = "atp_players_2000_train_test_MS_hard.csv",header = TRUE)
...
#loading the outputs of the M2_MS_hard
load("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_hard/gorgi_no_reg_nosurf_MS_hard_reduced_2_with_extra_burnin.RData")
...
p_post_preds_exp_win[,1] <- exp(lambda_new[,atp_matches_2000_test$player_i_id[1]])/(exp(lambda_new[,atp_matches_2000_test$player_i_id[1]])+exp(lambda_new[,atp_matches_2000_test$player_j_id[1]]))
...
for (t in 2:(nrow(atp_matches_2000_test))){
  ...
  p_post_preds_exp_win[,t] <- exp(lambda_pred[,atp_matches_2000_test$player_i_id[t],t])/(exp(lambda_pred[,atp_matches_2000_test$player_i_id[t],t])+exp(lambda_pred[,atp_matches_2000_test$player_j_id[t],t]))
}
