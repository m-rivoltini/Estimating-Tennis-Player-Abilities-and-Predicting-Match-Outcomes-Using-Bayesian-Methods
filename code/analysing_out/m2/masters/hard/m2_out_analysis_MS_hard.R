# ================= Analysing outputs of M2 applied to the hard data set for Masters ===========
...
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Analysing_model_fit_outputs/gorgi_noreg_nosurf/master/master_hard")

#loading outputs and obtained run times for each compilation
load(file = "/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_hard/gorgi_simple_model_output_MS_hard_reduced_2.RData")
run_time

load(file = "/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_hard/gorgi_no_reg_nosurf_MS_hard_reduced_2_with_extra_burnin.RData")
run_time_2

#importing data 
atp_matches_2000_train <- read.csv2("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_hard/atp_matches_2000_train_MS_hard_reduced_2.csv")
atp_players_2000_train <- read.csv2("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/master/master_hard/atp_players_2000_train_MS_hard_reduced_2.csv")
...
#saving summaries
write.csv(summaries,file = "M2_MS_hard_summaries.csv")
...
pdf("M2_MS_hard_seq_plots_top_players_ordered.pdf")
par(mfrow = c(3,1))
...
pdf("M2_MS_hard_lambda_trace_plots.pdf")
par(mfrow = c(3,1))
...
pdf("M2_MS_hard_lambda_density_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_MS_hard_p_ij_trace_plots.pdf")
par(mfrow = c(3,1))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_hard_p_ij_density_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_hard_lambda_acf_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_MS_hard_p_ij_acf_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_hard_lambda_gelman_plots.pdf")
#par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_MS_hard_p_ij_gelman_plots.pdf")
#par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_MS_hard_tau_trace_plots.pdf")
coda::traceplot(out$samples[,"tau"])
dev.off()

pdf("M2_MS_hard_tau_density_plots.pdf")
coda::densplot(out$samples[,"tau"])
dev.off()

pdf("M2_MS_hard_tau_acf_plots.pdf")
acf(out$sims.list$tau,main = "tau")
dev.off()

pdf("M2_MS_hard_tau_gelman_plots.pdf")
...
#Getting the probabilities of matches for some top players - note used
...
save(BIC,file = "M2_MS_hard_BIC.RData")
