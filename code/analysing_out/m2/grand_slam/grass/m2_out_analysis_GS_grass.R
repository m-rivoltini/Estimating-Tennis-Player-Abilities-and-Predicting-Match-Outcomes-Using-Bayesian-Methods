# ================= Analysing outputs of M2 applied to the grass data set for Grand Slam ===========
...
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Analysing_model_fit_outputs/gorgi_noreg_nosurf/grand_slam/grand_slam_grass")

#loading outputs and obtained run times for each compilation
load(file = "/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/grand_slam/grand_slam_grass/gorgi_simple_model_output_GS_grass.RData")
run_time

load(file = "/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/grand_slam/grand_slam_grass/gorgi_no_reg_nosurf_GS_grass_with_extra_burnin.RData")
run_time_2

#importing data 
atp_matches_2000_train <- read.csv2("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/grand_slam/grand_slam_grass/atp_matches_2000_train_GS_grass.csv")
atp_players_2000_train <- read.csv2("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/grand_slam/grand_slam_grass/atp_players_2000_train_GS_grass.csv")
...
player_names_opponents <- c(names_times,paste(atp_matches_2000_train$player_i_name,"vs",atp_matches_2000_train$player_j_name),"tau","NA") #NA here is for the deviance
...
#saving summaries
write.csv(summaries,file = "M2_GS_grass_summaries.csv")
...
top_players_ID <- order(lambda$av_strength,decreasing = TRUE)#[1:10]
...
pdf("M2_GS_grass_seq_plots_top_players_ordered.pdf")
par(mfrow = c(3,1))
...
pdf("M2_GS_grass_lambda_trace_plots.pdf")
par(mfrow = c(3,1))
...
pdf("M2_GS_grass_lambda_density_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_GS_grass_p_ij_trace_plots.pdf")
par(mfrow = c(3,1))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_GS_grass_p_ij_density_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_GS_grass_lambda_acf_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_GS_grass_p_ij_acf_plots.pdf")
par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_GS_grass_lambda_gelman_plots.pdf")
#par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
  for (i in 1:nrow(atp_players_2000_train)){
...
pdf("M2_GS_grass_p_ij_gelman_plots.pdf")
#par(mfrow = c(3,3))
for (t in 1:nrow(atp_matches_2000_train)){
...
pdf("M2_GS_grass_tau_trace_plots.pdf")
coda::traceplot(out$samples[,"tau"])
dev.off()

pdf("M2_GS_grass_tau_density_plots.pdf")
coda::densplot(out$samples[,"tau"])
dev.off()

pdf("M2_GS_grass_tau_acf_plots.pdf")
acf(out$sims.list$tau,main = "tau")
dev.off()

pdf("M2_GS_grass_tau_gelman_plots.pdf")
...
samples <- out$samples
param_samples <- samples[, paste("lambda[",top_players_ID[1],",",1,"]",sep = ""), drop = FALSE] 
gelman.plot(param_samples)
...
#Getting the probabilities of matches for some top players
...
#constructs the dataframe of matches of all matches of the top player - not used
...
#not_converged_player_strengths <- which(rhat_lambda > 1.1) #alla ibierek all Rhats converged >:((
...
BIC <- out$sims.list$deviance + (K*n+1)*log(n)
hist(BIC)

save(BIC,file = "M2_GS_grass_BIC.RData")
