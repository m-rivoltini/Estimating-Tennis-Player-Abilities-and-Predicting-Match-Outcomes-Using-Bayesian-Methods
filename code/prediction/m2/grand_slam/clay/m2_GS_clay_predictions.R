#This script file performs the predictions for out-of-sample matches for M2 on clay surface (Grand Slam Tour)
...
setwd("~/Library/CloudStorage/OneDrive-Personal/Uni Docs/Dissertation/R code/Predictions/gorgi_noreg_nosurf/grand_slam/grand_slam_clay")
...
#loading the outputs of the M2_GS_clay
load("/Volumes/SanDisk SSD/Model_fitting_V2/gorgi_noreg_nosurf/grand_slam/grand_slam_clay/gorgi_no_reg_nosurf_GS_clay_with_extra_burnin.RData")
...
for (t in 2:(nrow(atp_matches_2000_test))){
  s_i <- (as.numeric(atp_matches_2000_test$y[t-1])-1)*(1-p_post_preds_exp_win[,t-1]) - (1-(as.numeric(atp_matches_2000_test$y[t-1])-1))*p_post_preds_exp_win[,t-1] #doing this as.numeric()-1 since converting from factor to number changes factors to 1 and 2 (not 0 1). So subtract 1...
  s_j <- -s_i
  for (i in 1:nrow(atp_players_2000_train)){
